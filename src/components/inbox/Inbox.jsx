...
import { useSendMessageMutation, useGetMessagesQuery, useGetConversationsQuery } from "./inboxApi"
...
export default function Inbox() {
  ...
  const { activeConversation, conversations } = useSelector((state) => state.inbox);
  const [message, setMessage] = useState("");
  const { data: messages } = useGetMessagesQuery(activeConversation?.id);
  const { data: allConversations } = useGetConversationsQuery(user?.id, { skip: !user });
  ...
  async function SendMessageHandler() {
    if (message.trim() && activeConversation) {
      const newMessage = { senderId: userId, receiverId: activeConversation.userId, content: message };
    ...
    <div className="inboxMain">
      <aside className="conversations">
        ...
        {allConversations?.map((conversation) => (
          <div
            key={conversation.id}
            className={`convoCard ${activeConversation?.id === conversation.id ? "active" : ""}`}
      ...
      <article className="conversation">
        ...
        {messages?.map((msg) => (
          <p key={msg.id} className={msg.senderId === userId ? "sent message" : "received message"}>
            {msg.content}
          </p>
        ))}
        <div className="compose">
          ...
          />
          <button className="send" onClick={SendMessageHandler}>Send</button>
        </div>
      </article>
    </div>
  );
