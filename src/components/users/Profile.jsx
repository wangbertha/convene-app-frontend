      ...
      <section className="profile-main-card">
        ...
        <ul className="profile-list">
          ...
          <li className="profile-location-detail">
            <ProfileLocationDetail
              value={{ city: user.city, state: user.state }}
              stateOptions={stateOptions}
            />
          </li>
          <li>
            <ProfileOptionsDetail
              label="Age"
              type="age"
              value={user.age}
              options={ageOptions}
  ...
  return (
    <>
      <img
        ...
        src={value ? value : defaultPicture}
        alt="Your (logged-in user) profile picture"
      />
    ...
    <>
      <form onSubmit={sendUpdatePassword}>
        ...
        <div className="form-column-btns">
          {isEditing && <button type="submit">Save</button>}
          <button type="button" onClick={toggleIsEditing}>
            {!isEditing ? "Edit Password" : "Cancel"}
          </button>
        </div>
      </form>
      <p className="profile-response">{response}</p>
    ...
    <>
      <form onSubmit={sendUpdateMe}>
        ...
        <div className="form-column-btns">
          {isEditing && <button type="submit">Save</button>}
          <button type="button" onClick={toggleIsEditing}>
            {!isEditing ? "Edit Location" : "Cancel"}
          </button>
        </div>
      </form>
      <p className="profile-response">{response}</p>
...
function ProfileActiveDetail({ value }) {
  ...
  return (
    <>
      <form onSubmit={sendUpdateMe}>
        <label>
          ...
          {!isEditing ? (
            ...
            <select
              value={input}
              onChange={(e) => setInput(e.target.value === "true")}
            >
      ...
      <p className="profile-active-description">
        An active profile will be visible to other users, and will help you make
...
function ProfileInterestsDetail({ label, values }) {
  ...
  async function sendUpdateMyInterests(e) {
    ...
    try {
      ...
      const interestConnectMeResponse = await updateMe({
        interestToConnect: interestResponse.data,
      });
  ...
  async function sendDisconnectInterest(interest) {
    try {
      const disconnectResponse = await updateMe({
        interestToDisconnect: interest,
      });
        ...
        <ul className="list-bubbles">
          {values.map((interest) => (
            <li key={interest.id}>
              {interest.interest}
              <button
                className="list-bubble-delete"
                onClick={() => sendDisconnectInterest(interest)}
              >
                x
              </button>
            </li>
      ...
      {interests &&
        (input.length > 0 ? (
          <ul className="list-bubbles">
            {interests
              .filter(
                (interest) => interest.interest.slice(0, input.length) === input
              )
              .map((interest) => (
                <li
                  key={interest.id}
                  onClick={() => sendExistingInterest(interest)}
                >
                  {interest.interest}
                </li>
              ))}
  ...
  return (
    <>
      ...
      {values.length > 0 ? (
        <ul>
          {values.map((event) => (
            <ActivityCard key={event.id} activity={event} />
          ))}
        </ul>
      ) : (
        <p>
          You do not currently have any activities saved.{" "}
